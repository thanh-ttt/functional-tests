"""
Tests for $covarianceSamp numeric type mixing, overflow edge cases,
...
- $covarianceSamp divides by N-1; single element returns null
...
def test_covarianceSamp_all_int32_values(collection):
    """$covarianceSamp with all Int32 values produces Double result."""
    docs = [
...
def test_covarianceSamp_all_int64_values(collection):
    """$covarianceSamp with all Int64 values produces correct result."""
    docs = [
    ...
    expected = [
        {"_id": 1, "partition": "A", "x": Int64(1), "y": Int64(2), "result": 2.0},
        {"_id": 2, "partition": "A", "x": Int64(2), "y": Int64(4), "result": 2.0},
        {"_id": 3, "partition": "A", "x": Int64(3), "y": Int64(6), "result": 2.0},
    ]
    assertSuccess(result, expected, msg="all Int64 values compute correctly")


def test_covarianceSamp_mixed_int32_int64_double(collection):
    """$covarianceSamp with mixed Int32 + Int64 + Double in same frame."""
    docs = [
    ...
    expected = [
        {"_id": 1, "partition": "A", "x": 1, "y": 2.0, "result": 2.0},
        {"_id": 2, "partition": "A", "x": Int64(2), "y": 4, "result": 2.0},
        {"_id": 3, "partition": "A", "x": 3.0, "y": Int64(6), "result": 2.0},
    ]
...
def test_covarianceSamp_large_int64_near_max(collection):
    """$covarianceSamp with Int64 values near MAX_LONG — squaring would overflow 64-bit."""
    docs = [
        {
            "_id": 1,
            "partition": "A",
    ...
    # Both values round to the same float64 at this scale -> covSamp = 0.0
    expected = [
        {
            "_id": 1,
            "partition": "A",
...
def test_covarianceSamp_large_int64_spread(collection):
    ...
    # When x==y, covSamp(x,y) = varSamp(x) > 0
    checks = PerDoc(
        {"result": Gt(0)},
        {"result": Gt(0)},
        {"result": Gt(0)},
...
def test_covarianceSamp_very_large_value(collection):
    """$covarianceSamp with very large numeric value (1e308) — all identical returns NaN."""
    docs = [
...
def test_covarianceSamp_alternating_large_values(collection):
    """$covarianceSamp with alternating sign large values — stress accumulator."""
    docs = [
    ...
    # When x==y: covSamp = varSamp. Mean=0, varSamp = (4*1e30)/3 = 4e30/3
    expected = [
        {"_id": 1, "partition": "A", "x": 1e15, "y": 1e15, "result": 1.3333333333333332e30},
        {"_id": 2, "partition": "A", "x": -1e15, "y": -1e15, "result": 1.3333333333333332e30},
        {"_id": 3, "partition": "A", "x": 1e15, "y": 1e15, "result": 1.3333333333333332e30},
        {"_id": 4, "partition": "A", "x": -1e15, "y": -1e15, "result": 1.3333333333333332e30},
    ]
    assertSuccess(result, expected, msg="alternating large values produce correct covarianceSamp")


# Property [Algorithmic Precision]: known exact results and catastrophic cancellation handling


def test_covarianceSamp_known_exact_result(collection):
    """$covarianceSamp with known exact result: covSamp([1,2,3,4],[2,4,6,8]) = 10/3."""
    docs = [
    ...
    # covSamp = ((-1.5)(-3)+(-0.5)(-1)+(0.5)(1)+(1.5)(3))/3 = (4.5+0.5+0.5+4.5)/3 = 10/3
    expected = [
        {"_id": i, "partition": "A", "x": x, "y": y, "result": 3.3333333333333335}
        for i, (x, y) in enumerate([(1, 2), (2, 4), (3, 6), (4, 8)], 1)
    ]
    assertSuccess(result, expected, msg="covarianceSamp of [1,2,3,4],[2,4,6,8] must be 10/3")


def test_covarianceSamp_identical_values_exactly_zero(collection):
    """$covarianceSamp of identical (x,y) pairs where y is constant must be exactly 0.0."""
    docs = [
...
def test_covarianceSamp_catastrophic_cancellation(collection):
    """$covarianceSamp with large offset values — naive algorithm fails."""
    docs = [
    ...
    # covSamp = ((-1)(-2)+(0)(0)+(1)(2))/2 = 4/2 = 2.0
    expected = [
        {"_id": 1, "partition": "A", "x": 1000000001, "y": 1000000002, "result": 2.0},
        {"_id": 2, "partition": "A", "x": 1000000002, "y": 1000000004, "result": 2.0},
        {"_id": 3, "partition": "A", "x": 1000000003, "y": 1000000006, "result": 2.0},
    ]
    assertSuccess(
        result,
        expected,
        msg="catastrophic cancellation handled — correct covSamp for large offset values",
    )


def test_covarianceSamp_very_small_differences(collection):
    """$covarianceSamp with values that differ by very small amounts."""
    docs = [
    ...
    # The covarianceSamp should be positive and tiny
    checks = PerDoc(
    ...
    assertResult(
        result, expected=checks, msg="Small differences produce very small positive covSamp"
    )


# Property [Single Element Frame]: single value produces null for sample covariance


def test_covarianceSamp_single_element_sliding_window(collection):
    """$covarianceSamp returns null when sliding window frame has exactly one value."""
    docs = [
    ...
    # Window [0, 0] — each frame has exactly one value -> covSamp = null (N-1=0)
    expected = [
        {"_id": 1, "partition": "A", "x": 1, "y": 10, "result": None},
        {"_id": 2, "partition": "A", "x": 2, "y": 20, "result": None},
        {"_id": 3, "partition": "A", "x": 3, "y": 30, "result": None},
        {"_id": 4, "partition": "A", "x": 4, "y": 40, "result": None},
        {"_id": 5, "partition": "A", "x": 5, "y": 50, "result": None},
    ]
...
def test_covarianceSamp_pure_decimal128_values(collection):
    """$covarianceSamp with pure Decimal128 values returns Decimal128 type result."""
    docs = [
    ...
    # Server returns Decimal128 type: covSamp = 4/2 = 2
    expected = [
        {
            "_id": 1,
            "partition": "A",
...
def test_covarianceSamp_decimal128_with_double(collection):
    """$covarianceSamp with mixed Decimal128 and Double returns Decimal128 type."""
    docs = [
    ...
    expected = [
        {"_id": 1, "partition": "A", "x": Decimal128("1"), "y": 2.0, "result": Decimal128("2.0")},
        {"_id": 2, "partition": "A", "x": 2.0, "y": Decimal128("4"), "result": Decimal128("2.0")},
        {"_id": 3, "partition": "A", "x": Decimal128("3"), "y": 6.0, "result": Decimal128("2.0")},
    ]
    assertSuccess(result, expected, msg="mixed Decimal128 and Double returns Decimal128 type")


def test_covarianceSamp_decimal128_with_int32(collection):
    """$covarianceSamp with mixed Decimal128 and Int32 returns Decimal128 type."""
    docs = [
    ...
    # Server returns Decimal128 type when any input is Decimal128
    expected = [
        {"_id": 1, "partition": "A", "x": Decimal128("1"), "y": 2, "result": Decimal128("2")},
        {"_id": 2, "partition": "A", "x": 2, "y": Decimal128("4"), "result": Decimal128("2")},
        {"_id": 3, "partition": "A", "x": Decimal128("3"), "y": 6, "result": Decimal128("2")},
    ]
    assertSuccess(result, expected, msg="mixed Decimal128 and Int32 returns Decimal128 type")


def test_covarianceSamp_decimal128_with_int64(collection):
    """$covarianceSamp with mixed Decimal128 and Int64 returns Decimal128 type."""
    docs = [
...
def test_covarianceSamp_decimal128_all_types_mixed(collection):
    """$covarianceSamp with Decimal128 + Double + Int32 + Int64 all in same frame."""
    docs = [
        {"_id": 1, "partition": "A", "x": Decimal128("1"), "y": 2.0},
    ...
    # x=[1,2,3,4], y=[2,4,6,8] -> covSamp = 10/3
    # Server returns Decimal128 type when any input is Decimal128
    expected = [
        {
            "_id": 1,
            ...
            "x": Decimal128("1"),
            "y": 2.0,
            "result": Decimal128("3.333333333333333333333333333333333"),
        },
        {
            "_id": 2,
            "partition": "A",
...
def test_covarianceSamp_decimal128_sliding_window(collection):
    """$covarianceSamp with Decimal128 values in a sliding window."""
    docs = [
        {"_id": 1, "partition": "A", "x": Decimal128("1"), "y": Decimal128("2")},
        {"_id": 2, "partition": "A", "x": Decimal128("2"), "y": Decimal128("4")},
        {"_id": 3, "partition": "A", "x": Decimal128("3"), "y": Decimal128("6")},
    ...
    # Window [-1, 0]:
    # Row 1: [(1,2)] -> single pair -> null (N-1=0)
    # Row 2: [(1,2),(2,4)] -> covSamp = 1/1 = Decimal128("1")
    # Row 3: [(2,4),(3,6)] -> Decimal128("1")
    # Row 4: [(3,6),(4,8)] -> Decimal128("1")
    # Row 5: [(4,8),(5,10)] -> Decimal128("1")
    expected = [
        {"_id": 1, "partition": "A", "x": Decimal128("1"), "y": Decimal128("2"), "result": None},
        {
            "_id": 2,
            "partition": "A",
            "x": Decimal128("2"),
...
def test_covarianceSamp_decimal128_identical_values(collection):
    """$covarianceSamp with identical Decimal128 value pairs returns Decimal128('0E+12')."""
    docs = [
...
def test_covarianceSamp_decimal128_nan_special(collection):
    """$covarianceSamp with Decimal128 NaN — NaN is numeric and poisons the calculation."""
    docs = [
        {"_id": 1, "partition": "A", "x": Decimal128("1"), "y": Decimal128("2")},
    ...
    assertSuccessNaN(
        result, expected, msg="Decimal128 NaN is numeric and poisons covarianceSamp to NaN"
    )


def test_covarianceSamp_decimal128_infinity_special(collection):
    """$covarianceSamp with Decimal128 Infinity special value."""
    docs = [
        {"_id": 1, "partition": "A", "x": Decimal128("1"), "y": Decimal128("2")},
...
def test_covarianceSamp_decimal128_min_values(collection):
    """$covarianceSamp with DECIMAL128_MIN values — overflow in intermediate computation."""
    docs = [
...
def test_covarianceSamp_decimal128_large_exponent(collection):
    """$covarianceSamp with DECIMAL128_LARGE_EXPONENT values — high exponent Decimal128."""
    docs = [
    ...
    # x=[1E+6144, 2E+6144, 3E+6144], y=[2,4,6]: covSamp = 4/2 * 1E+6144 = 2E+6144
    expected = [
        {
            "_id": 1,
            "partition": "A",
            ...
            "y": Decimal128("2"),
            "result": Decimal128("2.000000000000000000000000000000000E+6144"),
        },
        {
            "_id": 2,
            "partition": "A",
    ...
    assertSuccess(result, expected, msg="DECIMAL128_LARGE_EXPONENT produces positive covSamp")


def test_covarianceSamp_decimal128_small_exponent(collection):
    """$covarianceSamp with DECIMAL128_SMALL_EXPONENT values — very small Decimal128."""
    docs = [
    ...
    # x=[1E-6143, 2E-6143, 3E-6143], y=[2,4,6]: covSamp = 4/2 * 1E-6143 = 2E-6143
    expected = [
        {
            "_id": 1,
            "partition": "A",
    ...
    assertSuccess(result, expected, msg="DECIMAL128_SMALL_EXPONENT produces positive covSamp")


def test_covarianceSamp_decimal128_negative_zero(collection):
    """$covarianceSamp with DECIMAL128_NEGATIVE_ZERO — treated as numeric zero."""
    docs = [
    ...
    # covSamp = ((-10)(-10)+(0)(0)+(10)(10))/2 = (100+0+100)/2 = 200/2 = 100.0
    expected = [
        {
            "_id": 1,
            "partition": "A",
...
def test_covarianceSamp_negative_zero(collection):
    """$covarianceSamp treats -0.0 as numeric zero — participates in computation."""
    docs = [
    ...
    # covSamp = ((-10)(-10)+(0)(0)+(10)(10))/2 = (100+0+100)/2 = 200/2 = 100.0
    expected = [
        {"_id": 1, "partition": "A", "x": DOUBLE_NEGATIVE_ZERO, "y": 10, "result": 100.0},
        {"_id": 2, "partition": "A", "x": 10, "y": 20, "result": 100.0},
        {"_id": 3, "partition": "A", "x": 20, "y": 30, "result": 100.0},
    ]
    assertSuccess(result, expected, msg="-0.0 treated as numeric zero in covarianceSamp")


# Property [Basic Numeric]: standard numeric inputs handled correctly


def test_covarianceSamp_negative_numbers(collection):
    """$covarianceSamp handles negative numbers correctly."""
    docs = [
    ...
    # covSamp = ((-10)(-20)+(0)(0)+(10)(20))/2 = (200+0+200)/2 = 400/2 = 200.0
    expected = [
        {"_id": 1, "partition": "A", "x": -10, "y": -20, "result": 200.0},
        {"_id": 2, "partition": "A", "x": 0, "y": 0, "result": 200.0},
        {"_id": 3, "partition": "A", "x": 10, "y": 20, "result": 200.0},
    ]
    assertSuccess(result, expected, msg="negative numbers handled correctly")


def test_covarianceSamp_decimals(collection):
    """$covarianceSamp handles floating-point (double) values."""
    docs = [
    ...
    # covSamp = ((-1)(-2)+(0)(0)+(1)(2))/2 = 4/2 = 2.0
    expected = [
        {"_id": 1, "partition": "A", "x": 1.5, "y": 3.0, "result": 2.0},
        {"_id": 2, "partition": "A", "x": 2.5, "y": 5.0, "result": 2.0},
        {"_id": 3, "partition": "A", "x": 3.5, "y": 7.0, "result": 2.0},
    ]
...
#                                    exactly 0 (n>=2; a single-element frame is
#                                    null via the n-1 divisor).
...
def test_covarianceSamp_identical_decimal128_max_three_docs(collection):
    """$covarianceSamp with 3 identical DECIMAL128_MAX x values -- deviations are exactly 0.

    Nothing in (1/(n-1))*sum((x_i - mean_x)(y_i - mean_y)) overflows: every x_i is the
...
def test_covarianceSamp_identical_decimal128_max_y_reversed(collection):
    """$covarianceSamp identical DECIMAL128_MAX x with descending y -- sign flips.
...
def test_covarianceSamp_identical_decimal128_max_two_docs(collection):
    """$covarianceSamp with 2 identical DECIMAL128_MAX x values -- no overflow yet.
...
def test_covarianceSamp_identical_decimal128_max_in_y(collection):
    """$covarianceSamp with identical DECIMAL128_MAX in the y position.
...
def test_covarianceSamp_identical_double_near_max_in_y(collection):
    """$covarianceSamp with identical DOUBLE_NEAR_MAX in y -- double path, y side.
...
def test_covarianceSamp_identical_double_near_max_two_docs(collection):
    """$covarianceSamp with 2 identical DOUBLE_NEAR_MAX x values -- returns 0.
...
def test_covarianceSamp_deviation_overflow_positive_decimal128(collection):
    """$covarianceSamp with x=y=[DECIMAL128_MAX, DECIMAL128_MIN] -- expects +Infinity.
...
def test_covarianceSamp_deviation_overflow_negative_decimal128(collection):
    """$covarianceSamp anti-correlated DECIMAL128 extremes -- expects -Infinity.
...
def test_covarianceSamp_deviation_overflow_positive_double(collection):
    """$covarianceSamp with x=y=[1e308, -1e308] -- double path, expects +Infinity.
...
def test_covarianceSamp_deviation_overflow_negative_double(collection):
    """$covarianceSamp anti-correlated 1e308 extremes -- double path, expects -Infinity."""
    docs = [
        {"_id": 1, "partition": "A", "x": DOUBLE_NEAR_MAX, "y": -DOUBLE_NEAR_MAX},
        {"_id": 2, "partition": "A", "x": -DOUBLE_NEAR_MAX, "y": DOUBLE_NEAR_MAX},
    ]
...
def test_covarianceSamp_deviation_overflow_representable_result(collection):
    """$covarianceSamp with x=[DECIMAL128_MAX, 0] -- large magnitude, exact result.

    Deviations reach half of DECIMAL128_MAX without overflowing, and the result
    -5.0E+6144 (the n-1 divisor doubles the population value) is representable, so
    it is returned exactly. This establishes that full-range magnitudes alone do
    not trigger the overflow cases above.
    """
    docs = [
        {"_id": 1, "partition": "A", "x": DECIMAL128_MAX, "y": Decimal128("1")},
    ...
    expected = [
        {**doc, "result": Decimal128("-5.000000000000000000000000000000000E+6144")} for doc in docs
    ]
    assertSuccess(
        result,
        expected,
...
def test_covarianceSamp_product_overflow_finite_deviations_double(collection):
    """$covarianceSamp with x=y=[1e200, -1e200] -- product overflows, deviations do not.
...
def test_covarianceSamp_product_overflow_finite_deviations_negative(collection):
    """$covarianceSamp anti-correlated 1e200 -- product overflows to -Infinity.
...
def test_covarianceSamp_product_overflow_finite_deviations_decimal128(collection):
    """$covarianceSamp with x=y=[1E+3100, -1E+3100] -- decimal product overflow.
...
def test_covarianceSamp_result_exceeds_decimal128_range(collection):
    """$covarianceSamp with x=y=DECIMAL128_MAX repeated -- server returns NaN.
