"""
Tests for $covarianceSamp argument validation in window context.
...
Server behavior (verified): $covarianceSamp does NOT reject bad argument
...
def test_covarianceSamp_two_field_paths(collection):
...
def test_covarianceSamp_operator_expressions(collection):
    """$covarianceSamp accepts operator expressions within the array."""
    docs = [
        {"_id": 1, "partition": "A", "x": 1, "y": 2},
        {"_id": 2, "partition": "A", "x": 2, "y": 4},
    ]
...
def test_covarianceSamp_same_field_both_positions(collection):
    """$covarianceSamp with same field in both positions equals varSamp."""
    docs = [
    ...
    expected = [
        {"_id": 1, "partition": "A", "x": 10, "result": 100.0},
        {"_id": 2, "partition": "A", "x": 20, "result": 100.0},
        {"_id": 3, "partition": "A", "x": 30, "result": 100.0},
    ]
    assertSuccess(result, expected, msg="same field both positions equals varSamp")


def test_covarianceSamp_literal_numeric_expressions(collection):
    """$covarianceSamp with literal numeric values — constant values produce covSamp=0."""
    docs = [
        {"_id": 1, "partition": "A", "x": 10},
        {"_id": 2, "partition": "A", "x": 20},
        {"_id": 3, "partition": "A", "x": 30},
    ...
    # All rows have same pair (5,10) -> covSamp = 0
    expected = [
        {"_id": 1, "partition": "A", "x": 10, "result": 0.0},
        {"_id": 2, "partition": "A", "x": 20, "result": 0.0},
        {"_id": 3, "partition": "A", "x": 30, "result": 0.0},
    ]
    assertSuccess(result, expected, msg="literal numeric expressions produce 0 covSamp")


# Property [Invalid Argument Shapes Return Null]: Server does NOT reject these at parse time


def test_covarianceSamp_single_expression_not_array_returns_null(collection):
    """$covarianceSamp with single field path (not array) returns null for all docs."""
    result = run_window_operator(
        collection,
        "$covarianceSamp",
        docs=TWO_DOCS,
...
def test_covarianceSamp_single_element_array_returns_null(collection):
    """$covarianceSamp with array of only 1 expression returns null for all docs."""
    result = run_window_operator(
        collection,
        "$covarianceSamp",
        docs=TWO_DOCS,
...
def test_covarianceSamp_three_element_array_returns_null(collection):
    """$covarianceSamp with array of 3 expressions returns null for all docs."""
    result = run_window_operator(
        collection,
        "$covarianceSamp",
        docs=TWO_DOCS,
...
def test_covarianceSamp_empty_array_returns_null(collection):
    """$covarianceSamp with empty array [] returns null for all docs."""
    result = run_window_operator(
        collection,
        "$covarianceSamp",
        docs=TWO_DOCS,
...
def test_covarianceSamp_object_expression_returns_null(collection):
    """$covarianceSamp with object (not array) as argument returns null for all docs."""
    result = run_window_operator(
        collection,
        "$covarianceSamp",
        docs=TWO_DOCS,
...
def test_covarianceSamp_unknown_key_in_output_field_errors(collection):
    """Unknown key alongside $covarianceSamp in output field spec produces parse error."""
    ...
    assertFailureCode(
        result, FAILED_TO_PARSE_ERROR, msg="unknown key alongside $covarianceSamp rejected"
    )


def test_covarianceSamp_unknown_key_errors_on_empty_collection(collection):
...
def test_covarianceSamp_multiple_accumulators_in_output_field_errors(collection):
...
def test_covarianceSamp_no_accumulator_in_output_field_errors(collection):
...
def test_covarianceSamp_unrecognized_expression_operator_in_array_errors(collection):
    """$covarianceSamp with unrecognized expression operator in array element produces error."""
    collection.insert_many(SINGLE_DOC)
    result = execute_command(
        collection,
        {
...
def test_covarianceSamp_field_path_empty_component_errors(collection):
    """$covarianceSamp with field path containing empty component produces error."""
    collection.insert_many(SINGLE_DOC)
    result = execute_command(
        collection,
        {
...
def test_covarianceSamp_multi_field_expression_object_errors(collection):
    """$covarianceSamp with multi-field expression object in array element produces error."""
    collection.insert_many(SINGLE_DOC)
    result = execute_command(
        collection,
        {
