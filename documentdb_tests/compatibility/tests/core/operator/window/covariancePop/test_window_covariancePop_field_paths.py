"""
Tests for $covariancePop with nested field paths, array field traversal,
...
$covariancePop takes two expressions: ["$x", "$y"]. These tests exercise
...
# Tests that $covariancePop correctly accesses nested document values via dotted paths.


def test_covariancePop_dotted_field_path(collection):
    """$covariancePop with dotted field path accesses nested document value."""
    docs = [
...
def test_covariancePop_missing_intermediate_path_x(collection):
    """$covariancePop with missing intermediate path in first expression (x) — row ignored."""
    docs = [
    ...
    expected = [
        {"_id": 1, "partition": "A", "data": {"x": 1}, "y": 2, "result": 2.0},
        {"_id": 2, "partition": "A", "y": 4, "result": 2.0},
        {"_id": 3, "partition": "A", "data": {"x": 3}, "y": 6, "result": 2.0},
    ]
    assertSuccess(result, expected, msg="missing intermediate path in x -> row ignored")


def test_covariancePop_missing_intermediate_path_y(collection):
    """$covariancePop with missing intermediate path in second expression (y) — row ignored."""
    docs = [
    ...
    # mean_x=2, mean_y=4, covPop = ((-1)(-2)+(1)(2))/2 = 4/2 = 2.0
    expected = [
        {"_id": 1, "partition": "A", "x": 1, "data": {"y": 2}, "result": 2.0},
        {"_id": 2, "partition": "A", "x": 2, "data": {"other": 99}, "result": 2.0},
        {"_id": 3, "partition": "A", "x": 3, "data": {"y": 6}, "result": 2.0},
    ]
    assertSuccess(result, expected, msg="missing intermediate path in y -> row ignored")


def test_covariancePop_top_level_missing_object(collection):
    """$covariancePop where the top-level field of a dotted path is missing."""
    docs = [
        {"_id": 1, "partition": "A", "data": {"x": 1, "y": 2}},
...
def test_covariancePop_nested_field_explicit_null(collection):
    """$covariancePop with nested field that exists but is null — row ignored."""
    docs = [
        {"_id": 1, "partition": "A", "data": {"x": 1}, "y": 2},
    ...
    expected = [
        {"_id": 1, "partition": "A", "data": {"x": 1}, "y": 2, "result": 2.0},
        {"_id": 2, "partition": "A", "data": {"x": None}, "y": 4, "result": 2.0},
        {"_id": 3, "partition": "A", "data": {"x": 3}, "y": 6, "result": 2.0},
    ]
...
def test_covariancePop_array_field_is_non_numeric(collection):
    """$covariancePop on a top-level array field — arrays are non-numeric, should be ignored."""
    docs = [
    ...
    # Only doc 2 has numeric x. Single numeric pair -> covPop = 0
    expected = [
        {"_id": 1, "partition": "A", "x": [1, 2, 3], "y": 10, "result": 0.0},
        {"_id": 2, "partition": "A", "x": 5, "y": 20, "result": 0.0},
        {"_id": 3, "partition": "A", "x": [4, 5], "y": 30, "result": 0.0},
    ]
...
def test_covariancePop_array_of_objects_traversal(collection):
    """$covariancePop with path traversing array-of-objects — resolves to array, ignored."""
    docs = [
    ...
    # mean_x=10, mean_y=300, covPop = ((-5)(-100)+(5)(100))/2 = 1000/2 = 500.0
    expected = [
        {
            "_id": 1,
            "partition": "A",
        ...
        {"_id": 2, "partition": "A", "items": {"value": 5}, "y": 200, "result": 500.0},
        {
            "_id": 3,
            "partition": "A",
        ...
        {"_id": 4, "partition": "A", "items": {"value": 15}, "y": 400, "result": 500.0},
    ]
...
def test_covariancePop_expression_returns_different_types(collection):
    """$covariancePop expression returning different types per row — non-numeric ignored."""
    docs = [
    ...
    # mean_x=20, mean_y=40, covPop = ((-10)(-20)+(10)(20))/2 = (200+200)/2 = 200.0
    expected = [
        {"_id": 1, "partition": "A", "x": 10, "y": 20, "result": 200.0},
        {"_id": 2, "partition": "A", "x": -5, "y": 40, "result": 200.0},
        {"_id": 3, "partition": "A", "x": 30, "y": 60, "result": 200.0},
        {"_id": 4, "partition": "A", "x": -1, "y": 80, "result": 200.0},
    ]
...
def test_covariancePop_expression_returns_null_for_some(collection):
    """$covariancePop expression returning null for some docs, number for others."""
    docs = [
    ...
    # mean_x=40, mean_y=40, covPop = ((-20)(-20)+(20)(20))/2 = (400+400)/2 = 400.0
    expected = [
        {"_id": 1, "partition": "A", "x": 10, "y": 20, "factor": 2, "result": 400.0},
        {"_id": 2, "partition": "A", "x": 20, "y": 40, "factor": None, "result": 400.0},
        {"_id": 3, "partition": "A", "x": 30, "y": 60, "factor": 2, "result": 400.0},
    ]
...
def test_covariancePop_date_value_as_expression_ignored(collection):
    """$covariancePop with Date value in one expression field — non-numeric, ignored."""
    docs = [
    ...
    # Only doc 2 is valid. Single pair -> covPop = 0
    expected = [
        {
            "_id": 1,
            "partition": "A",
        ...
        {"_id": 2, "partition": "A", "x": 20, "y": 40, "result": 0.0},
        {
            "_id": 3,
            "partition": "A",
...
def test_covariancePop_numeric_path_component(collection):
    """$covariancePop with numeric path component accesses array element or object key."""
    docs = [
